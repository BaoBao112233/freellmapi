...
const arg = process.argv.find((a) => a.startsWith('--drawin-token='));
if (arg) {
  try {
    window.localStorage.setItem(TOKEN_KEY, arg.slice('--drawin-token='.length));
  } catch {
...
contextBridge.exposeInMainWorld('__DRAWIN_DESKTOP__', true);
...
function reportLocale() {
  ...
  if (lang) ipcRenderer.send('drawin:locale-changed', lang);
}
