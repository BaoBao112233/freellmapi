...
// chat ModelDetailPage for the image, audio, and video modalities.
export default function MediaDetailPage({ modality }: { modality: 'image' | 'audio' | 'video' }) {
  ...
  -d '{
    ...
    // Video generation is slow — allow a long read timeout.
    : `curl --max-time 600 ${base}/videos/generations \\
  -H "Authorization: Bearer ${key}" \\
  -H "Content-Type: application/json" \\
  -d '{
    "model": "${exampleModel}",
